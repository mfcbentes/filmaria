<template>
  <div id="meus-filmes">
    <h1>Meus filmes</h1>
    <span v-if="filmes.length <= 0">Você não possui nenhum filme.</span>
    <ul>
      <li v-for="filme in filmes" v-bind:key="filme.id">
        <span>{{ filme.nome }}</span>
        <div>
          <router-link v-bind:to="`/filme/${filme.id}`" tag="button"
            >Ver detalhes</router-link
          >
          <button v-on:click="excluirFilme(filme.id)">Excluir</button>
        </div>
      </li>
    </ul>
  </div>
